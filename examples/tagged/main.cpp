...
int main() {
    Tagged<Vector2<int>> v({5, 2}), u({4, 2}); // Vectors with no tag
    std::cout << "v = " << to_str(v) << ", u = " << to_str(u) << "\n";

    auto t = v;
    Tagged<Vector2<int>> tt(v);
    std::cout << "assign = " << to_str(t) << ", copy = " << to_str(tt) << "\n";

    std::cout << "u + v =" << to_str(u + v) << "\n";

    using V2T = Tagged<Vector2<int>, int, OperatorMerge<int>>;
    auto a = V2T::make_tagged(42, Vector2<int>{{1, 1}});
    auto b = V2T::make_tagged(10, Vector2<int>{{-1, -1}});
    ...
    using V2T2 = Tagged<Vector2<int>, int, KeepLeftMerge<int>>;
    auto c = V2T2::make_tagged(1, Vector2<int>{{1, 1}});
